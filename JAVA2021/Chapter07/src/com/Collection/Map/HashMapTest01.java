...
HashMap:
    ...
    5. map.get(k)实现原理：
        先调用k的hashCode()方法得出哈希值，通过哈希函数/哈希算法转换成数组下标，通过数组下标快速定位到某个位置上，
    ...
    8. 假设hashCode()方法返回一个固定值，那么会导致哈希表变成了纯单向链表；假设hashCode()方法返回的值都是不一样的值
       那么会导致哈希表变成了纯一维数组。这种情况称为：散列分布不均匀
        什么是散列分布均匀？
        假设有100个元素，10个单向链表，那么每个单向链表上有10个节点，这是最好的
    9. 重点：放在HashMap中的key、放在HashSet中的元素需要同时重写hashCode()和equals()方法
    10. HashMap的数组默认初始化容量是16，默认加载因子是0.75
        这个默认加载因子是当HashMap底层数组的容量达到75%时，数组开始扩容，扩容之后的容量是原来的2倍
        HashMap的数组默认初始化容量必须是2的倍数，这是为了达到散列均匀分布所必要的
    11. 在JDK8.0之后，哈希表的单向链表中元素个数超过8个，单向链表这种数据结构会变成红黑树；
        当红黑树上的节点数量小于6时，会重新把红黑树变成单向链表数据结构
    12. 解决冲突：①开放定址法。如线性探测、平方探测；
                ②链接法。链表，java采用这种方法。
 */
